...
mongoose
  .connect("mongodb://127.0.0.1:27017/appMallDemo", { useNewUrlParser: true })
  .then(() => {
...
app.patch("/api/employees/:id", (req, res) => {
  ...
  Employee.findByIdAndUpdate(employeeId, updatedEmployeeData)
    .then((data) => {
      ...
      res
        .status(200)
        .json({ message: "Employee updated successfully", employee: data });
    })
    .catch((_) => res.status(500).json({ error: "Error updating employee" }));
});
...
app.delete("/api/employees/:id", (req, res) => {
  ...
  Employee.findByIdAndRemove(employeeId)
    .then((data) => {
      ...
      res
        .status(200)
        .json({ message: "Employee deleted successfully", employee: data });
    })
    .catch((error) => {
...
app.patch("/api/product/:id", (req, res) => {
  ...
  Product.findByIdAndUpdate(productId, updatedProductData)
    ...
    .catch((_) => res.status(500).json({ error: "Error updating product" }));
});
...
app.delete("/api/product/:id", (req, res) => {
  ...
  Product.findByIdAndRemove(productId)
    .then((data) => {
      ...
      res
        .status(200)
        .json({ message: "Product deleted successfully", product: data });
    })
    .catch((_) => res.status(500).json({ error: "Error deleting product" }));
});
